...
export default function WinRateDashboard() {
  ...
  const fetchLivePrices = useCallback(async (trackedSignals: SignalRecord[]) => {
  ...
  return (
    <div className="w-full space-y-6 bg-gray-900 text-white p-6 rounded-xl border border-gray-800">
      ...
      <div className="flex items-center justify-between border-b border-gray-800 pb-4">
        <h2 className="text-xl font-bold text-gray-100">Performance & Win-Rate Tracker (v5.1.1)</h2>
        <button
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-gray-300">
          ...
          <tbody className="divide-y divide-gray-800">
            {signals.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-4 text-center text-gray-500">No signals tracked yet.</td>
              </tr>
            ) : (
              signals.map((sig) => {
                    ...
                    <td className="py-3 px-4 font-mono font-bold">
                      {currentPrice ? (
                        <div className="flex items-center space-x-2">
                          <span className="text-cyan-400">${formatPrice(currentPrice)}</span>
                          {priceDiff !== null && (
